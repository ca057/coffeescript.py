...
def get_image_files(input_folder, file_endings=None):
    ...
    def is_image_file(file_path=""):
        return path.isfile(file_path) and any((file_path.lower().endswith(fe.lower()) for fe in file_endings))

    image_file_list = [f for f in listdir(
        input_folder) if is_image_file(path.join(input_folder, f))]
...
def create_output_folder_if_not_exists(output_folder):
    if not path.exists(output_folder):
        ...
        raise EnvironmentError(
            "a file at the path {0} exists, path cannot be used for the output folder".format(output_folder))
...
def main(args):
    ...
    input_image_files.sort(key=lambda img_info: img_info['data'][args.key], reverse=args.reverse)

    columns = math.ceil(math.sqrt(len(input_image_files))) if args.columns is None else args.columns
...
if __name__ == "__main__":
    ...
    parser.add_argument('-k', '--key', default="date_time", help="key to sort the images by, date_time or brightness")
    ...
    parser.add_argument('-s', '--size', default=1000, help="width/height for each image in compilation")
    parser.add_argument('-m', '--margin', default=50, help="margin between two images in compilation")
